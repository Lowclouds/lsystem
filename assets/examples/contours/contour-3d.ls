/* 
   ...
   Contour with multiplicty 4, with each quadrant a Hermite spline. We turn this red using 
   the programming statement section of the production, changing lcolor to red if the 
   variable c == 5 in production L.
*/
#define STEPL 0.5
#define STEPA 3
#define LEAFL 30
...
L : * {c = c + 1; w = (c==5) ? 0.7 : STEPW; lcolor = (c==5) ? 153 : lcolor} --> [-(30)@v;(lcolor)@#(c){(0)A(1,w)}(0)]
...
C(i) : i == 1 --> @CsD(i)@Ce(i)@m(0,0,0)
D(i) : i == 1 --> @m(-3,-1,0).@m(-2.7,-0.3,0).@m(-2,0,0).@m(2,0,0).@m(2.7,-0.3,0).@m(3,-1,0).3
C(i) : i == 2 --> @CsD(i)@Ce(i)@m(0,0,0)
D(i) : i == 2 --> @m(-3,-1,0).@m(-2,0,0).@m(3,-1,0).@Ca(0)
C(i) : i == 3 --> @CsD(i)@Ce(i)@m(0,0,0)
D(i) : i == 3 --> @m(0,-3,0).@m(-3,-1,0).@Ca(1,90)
C(i) : i == 4 --> @CsD(i)@Ce(i)@m(0,0,0)
D(i) : i == 4 --> @m(-3,-1,0)@R(1,1,0).@m(3,-1,0)@R(1,-1,0).@Ct
C(i) : i == 5 --> @Cs(24,1)D(i)@Ce(i)@m(0,0,0)M
D(i) : i == 5 --> @m(-3,1,0)@R(-1,1,0).@m(3,1,0)@R(1,1,0).@Ct@Cm(4)
